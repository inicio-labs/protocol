...
- [BREAKING] Replaced the `SwapNote`, `BurnNote`, and `MintNote` marker types and their `::create` factories with structs built via `bon` typestate builders (`SwapNote::builder()`, `BurnNote::builder()`, `MintNote::builder()`). `BurnNote` and `MintNote` convert into a `Note` via `Note::from`; a `SwapNote` yields the swap note and its payback `NoteDetails` via `SwapNote::into_notes`. The builders offer `.attachment()`/`.attachments()` and serial-number generation ([#2283](https://github.com/0xMiden/protocol/issues/2283)).
